...
let UsuarioController = class UsuarioController {
    ...
    async recuperarClave(usuarioRecuperacion) {
        const p = await this.servicioAutenticacion.RecuperarClaveUsuario(usuarioRecuperacion.Usuario);
        if (p) {
            ...
            fetch(`${llaves_1.Llaves.urlServicioNotificaciones}/email?destinatario=${destino}&asunto=${asunto}&mensaje=${contenido}`).then((data) => {
                console.log(data);
            });
            return p;
        }
    ...
    async cambiarClave(cambioClave) {
        const p = await this.servicioAutenticacion.CambiarClaveUsuario(cambioClave.Usuario, cambioClave.ClaveActual);
        if (p) {
